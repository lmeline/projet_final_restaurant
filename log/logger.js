...
eventBus.on('reservation:create:success', ({reservationId, email, ip, numberOfPeople, date, time }) => {
    writeLog(`[reservation:create:success] | reservation:create:success | reservation_id '${reservationId}' | '${email}' from ip adress [${ip}] | numberOfPeople '${numberOfPeople}' | date '${date}' | time '${time}'`);
})
eventBus.on('reservation:create:failure', ({StatusCode, error}) => {
    writeLog(`[reservation:create:failure] | reservation:create:failure | StatusCode '${StatusCode}' | error '${error}'`);
});

// Routes validation réservation
eventBus.on('reservation:validate:failure', ({StatusCode, error }) => {
    writeLog(`[reservation:validate:failure] | reservation:validate:failure | StatusCode '${StatusCode}' | error '${error}'`);
});
eventBus.on('reservation:validate:success', ({reservationId }) => {
    writeLog(`[reservation:validate:success] | reservation:validate:success | reservation_id '${reservationId}'`);
});

// Routes annulation réservation
eventBus.on('reservation:cancel:success', ({reservationId }) => {
    writeLog(`[reservation:cancel:success] | reservation:cancel:success | reservation_id '${reservationId}'`);
});
eventBus.on('reservation:cancel:failed', ({StatusCode, error }) => {
    writeLog(`[reservation:cancel:failed] | reservation:cancel:failed |StatusCode '${StatusCode}' | error '${error}'`);
});

// Routes modification réservation
eventBus.on('reservation:modify:success', ({reservationId,numberOfPeople,date,time, email, ip }) => {
    writeLog(`[reservation:modify:success] | reservation:modify:success | reservation_id '${reservationId}' | numberOfPeople '${numberOfPeople}' | date '${date}' | time '${time}' | '${email}' from ip adress [${ip}]`);
});
eventBus.on('reservation:modify:failure', ({StatusCode, error,}) => {
    writeLog(`[reservation:modify:failure] | reservation:modify:failure | StatusCode '${StatusCode}' | error '${error}'`);
});

// Routes récupération de mes reservations
eventBus.on('reservation:my-reservations:success', ({user_id}) => {
    writeLog(`[reservation:my-reservations:success] | reservation:my-reservations:success | user_id '${user_id}'`)
});
eventBus.on('reservation:my-reservations:failure', ({StatusCode, error}) => {
    writeLog(`[reservation:my-reservations:failure] | reservation:my-reservations:failure | StatusCode '${StatusCode}' | error '${error}'`)
});
